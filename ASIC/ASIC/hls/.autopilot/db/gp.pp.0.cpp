...
#pragma HLS LOOP_TRIPCOUNT min=1 max=320
 VITIS_LOOP_51_2: for (int c = 0; c < MAX_TOTAL_COLS; ++c) {
    ...
    VITIS_LOOP_56_3: for (int r = 0; r < rows; ++r) {
        VITIS_LOOP_57_4: for (int c = 0; c < cols; ++c) {
...
#pragma HLS INLINE off
 int rows = M.rows;
    int cols = M.cols;
    VITIS_LOOP_68_1: for (int r = 0; r < rows; ++r) {
        VITIS_LOOP_69_2: for (int c = 0; c < cols; ++c) {
...
#pragma HLS INLINE
 int total_rows = M.rows + M.t;
    int cnt = 0;
    VITIS_LOOP_81_1: for (int r = 0; r < total_rows; ++r) {
...
#pragma HLS INLINE
 int total_cols = M.cols + M.t;
    int s = 0;
    VITIS_LOOP_92_1: for (int c = 0; c < total_cols; ++c) {
...
#pragma HLS INLINE off
 pp = 0; nn = 0;
    ...
    VITIS_LOOP_104_1: for (int c = 0; c < cols_non_t; ++c) {
...
#pragma HLS INLINE
 if (M.t >= M.t_capacity) return;
    ...
    VITIS_LOOP_124_1: for (int c = 0; c < existing_cols; ++c) {
...
#pragma HLS INLINE
 if (M.t <= 0) return;
    ...
    VITIS_LOOP_146_1: for (int r = 0; r < rowt; ++r) {

        elem_t v = M.e[r][colt];
        if (v == 0) continue;
        if (v == 1) { row1 = r; break; }
    }
    VITIS_LOOP_152_2: for (int r = rowt - 1; r > row1; --r) {
    ...
    M.e[row1][colt] = 0;
    M.e[row2][colt] = 0;
    VITIS_LOOP_163_3: for (int c = 0; c < colt; ++c) {
...
#pragma HLS INLINE
 int R = M.rows + M.t;
    int s = 0;
    VITIS_LOOP_180_1: for (int i = 0; i < R; ++i) {
        VITIS_LOOP_181_2: for (int j = i+1; j < R; ++j) {
...
#pragma HLS INLINE off
 score_t best_score = 0;
    int R = M.rows + M.t;
    VITIS_LOOP_212_1: for (int i = 0; i < R; ++i) {
        VITIS_LOOP_213_2: for (int j = i+1; j < R; ++j) {
...
#pragma HLS INLINE off
VITIS_LOOP_232_1: for (int iter = 0; iter < MAX_ITERS; ++iter) {
...
extern "C" __attribute__((sdx_kernel("fmm_reduce_kernel", 0))) void fmm_reduce_kernel(int32_t *A_dram,
                                  int rows,
                                  int cols,
                                  int t_capacity,
                                  int k1,
