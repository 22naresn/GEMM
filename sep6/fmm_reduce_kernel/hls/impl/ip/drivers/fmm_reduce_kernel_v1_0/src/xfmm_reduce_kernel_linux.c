...
typedef struct {
    ...
    XFmm_reduce_kernel_uio_map maps[ MAX_UIO_MAPS ];
} XFmm_reduce_kernel_uio_info;

/***************** Variable Definitions **************************************/
static XFmm_reduce_kernel_uio_info uio_info;
...
static int uio_info_read_name(XFmm_reduce_kernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFmm_reduce_kernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFmm_reduce_kernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFmm_reduce_kernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFmm_reduce_kernel_Initialize(XFmm_reduce_kernel *InstancePtr, const char* InstanceName) {
	XFmm_reduce_kernel_uio_info *InfoPtr = &uio_info;
