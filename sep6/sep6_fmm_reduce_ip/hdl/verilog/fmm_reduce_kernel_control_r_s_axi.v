...
// ==============================================================
`timescale 1ns/1ps
module fmm_reduce_kernel_control_r_s_axi
#(parameter
...
localparam
    ...
    ADDR_DEBUG_DRAM_DATA_0 = 6'h1c,
    ADDR_DEBUG_DRAM_DATA_1 = 6'h20,
    ADDR_DEBUG_DRAM_CTRL   = 6'h24,
    WRIDLE                 = 2'd0,
    WRDATA                 = 2'd1,
    WRRESP                 = 2'd2,
    WRRESET                = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_DEBUG_DRAM_DATA_0: begin
                    rdata <= int_debug_dram[31:0];
                end
                ADDR_DEBUG_DRAM_DATA_1: begin
                    rdata <= int_debug_dram[63:32];
                end
            endcase
        end
    end
...
assign debug_dram = int_debug_dram;
// int_A_dram[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_A_dram[31:0] <= 0;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_debug_dram[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_DEBUG_DRAM_DATA_0)
            int_debug_dram[31:0] <= (WDATA[31:0] & wmask) | (int_debug_dram[31:0] & ~wmask);
    end
end

// int_debug_dram[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_debug_dram[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_DEBUG_DRAM_DATA_1)
            int_debug_dram[63:32] <= (WDATA[31:0] & wmask) | (int_debug_dram[63:32] & ~wmask);
    end
end
